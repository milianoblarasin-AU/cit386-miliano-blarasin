...
I recommend **Azure**. The deciding requirement is that the ordering application must remain publicly reachable overnight without depending on equipment or an employee inside the bakery. Every local option ties availability to one device, the storefront's power, and the storefront's internet connection. Azure moves those dependencies away from the bakery while also supporting seasonal growth and a configuration within the $180 monthly limit. The owner should use multifactor authentication, least-privilege access, automated backups, monitoring, and cost alerts to control the risks identified above.
